...
flowchart TD
    ...
    subgraph GCP["GCP Project: portfolio-472821"]
        subgraph CloudRunGroup[Cloud Run Services]
            FE["Cloud Run<br/>Frontend<br/>（外部公開）<br/>egress: PRIVATE_RANGES_ONLY"]
            BE["Cloud Run<br/>Backend<br/>（外部公開）<br/>egress: PRIVATE_RANGES_ONLY"]
            AI["Cloud Run<br/>AI Service<br/>（内部専用）<br/>egress: ALL_TRAFFIC<br/>ingress: INTERNAL_ONLY"]
        end

        subgraph Network[VPC: trip-shiori-dev-vpc]
            Subnet["Subnetwork<br/>10.0.0.0/24<br/>(Direct VPC Egress用)"]
            ServiceNetworking["Service Networking<br/>Private Service Connect"]
            DNS["Private DNS Zone<br/>run.app."]
            Router["Cloud Router<br/>NAT Router"]
            NAT["Cloud NAT<br/>AI Service用"]
            Firewall["ファイアウォール<br/>egress許可 (80,443,53)"]
        end

        subgraph Secrets[Secret Manager]
            SM["機密情報管理<br/>DB認証情報・APIキー・JWT秘密鍵"]
        end
        ...
        SQL[("Cloud SQL<br/>PostgreSQL 16<br/>Private IP")]
        GCS["Cloud Storage<br/>Static Assets<br/>(将来的な静的配布用)"]
    ...
    BE -->|Direct VPC Egress| Subnet
    AI -->|Direct VPC Egress| Subnet
    AI -->|Cloud NAT経由| NAT
    NAT -->|外部API呼び出し| ExternalAPIs

    Subnet -->|Private Service Connect| ServiceNetworking
    ServiceNetworking -->|Cloud SQL Private IP を提供| SQL
    BE -.->|論理的な接続（接続文字列でPrivate IPに到達）| SQL
...
| VPC / Subnetwork | Direct VPC Egress用のプライベート通信を確保。Cloud SQLを外部から切り離す | `google_compute_network.main`, `google_compute_subnetwork.main` |
...
- **VPCアクセス設定**: 
  ...
  - AIサービス: `egress = "ALL_TRAFFIC"` で外部API（OpenAI/Cerebras/Tavily）へのアクセスを許可
- **外部APIアクセス**: AIサービスはDirect VPC Egress + Cloud NAT経由で外部APIにアクセス。ファイアウォールルールでHTTPS/HTTP/DNSのみ許可。
- **Cloud SQL接続**: BackendはDirect VPC Egress経由のプライベートIP接続でアクセス。Service Networkingで`SERVICENETWORKING`APIのReserved Rangeを活用。
...
- **Cloud Run backend ↔ Cloud SQL**: 接続文字列はPrivate IPを参照し、VPC内経路のみで双方向通信（例: `postgresql://trip_shiori_user:***@10.0.x.x:5432/trip_shiori`）。Cloud SQLのパブリックIPv4は無効化。
- **Cloud Run services ↔ Secret Manager**: 機密情報（DB認証情報、APIキー、JWT秘密鍵）を動的に取得。IAMロールベースのアクセス制御。
- **AI Service → Cloud NAT → 外部API**: OpenAI、Cerebras、Tavily APIへのHTTPS通信。Direct VPC Egress + Cloud NAT + ファイアウォールルールにより外部アクセスを許可。
...
- **外部APIアクセス**: AWSではNAT Gateway/Instanceで外部アクセスを制御するが、GCPではCloud NAT + ファイアウォールルールで同様の機能を提供。Direct VPC Egressにより、よりシンプルな構成で外部APIアクセスが可能。
...
| Cloud NAT | AWS NAT Gateway | プライベートIPから外部へのアクセスを可能にするNATサービス |
...
- **Q: AIサービスはなぜ内部専用にしたのですか？**  
  A: セキュリティ向上のため。`ingress = "INGRESS_TRAFFIC_INTERNAL_ONLY"`により外部からの直接アクセスを防止し、Backendからの内部HTTP通信のみ許可。Terraformのネットワーク制御により保護されている。
- **Q: Cloud Run から Cloud SQL への通信はどう守っていますか？**  
  A: Cloud SQL のパブリック IPv4 は無効化し、Direct VPC Egress 経由の Private IP のみに限定している。
- **Q: 機密情報はどう管理していますか？**  
  A: Secret ManagerでDB認証情報、APIキー、JWT秘密鍵を安全に管理。各サービスがIAMロールベースで動的に取得し、環境変数にハードコードしない。
- **Q: VPC/Subnet はどこで使っていますか？**  
  A: Cloud Run 自体は VPC に常駐しないが、Direct VPC Egress 経由で VPC に接続し、Cloud SQL Private IP にアクセスする。VPC/Subnet は主にこの経路確保と外部APIアクセス（Cloud NAT）のため。
- **Q: AIサービスの外部APIアクセスはどう設定していますか？**  
  A: AIサービスは`egress = "ALL_TRAFFIC"`設定 + Direct VPC Egress + Cloud NAT + ファイアウォールルールにより外部API（OpenAI/Cerebras/Tavily）へのアクセスを許可。Backend/Frontendは`PRIVATE_RANGES_ONLY`でVPC内リソースのみアクセス。
...
- **Q: 将来的に Compute Engine や GKE を追加する想定は？**  
  A: カスタム VPC/Subnet が既にあるので、必要になればそのレンジ内に VM/ノードを追加できる拡張余地がある。Cloud NATも既に設定済みなので、外部APIアクセスも可能。
- **Q: Cloud Run の IP や証明書はどう管理していますか？**  
  A: Cloud Run のエンドポイントは Google が管理し、IP は動的。カスタムドメインを設定すれば証明書更新も含め自動で面倒を見てくれるので静的 IP は不要。
- **Q: 外部公開しているのはどこまで？**  
  A: 外部から到達できるのは Cloud Run frontend/backend の HTTPS のみ。AIサービスは内部専用、Cloud SQL は Private IP のみ、バケットやレジストリは IAM で制御しつつ公開 API 経由で利用する。
...
> NOTE: 本図はTerraform定義（2025-01-15 時点）を基に作成しています。環境固有の設定値（プロジェクト名やリージョン）は `terraform/environments/` の変数を参照してください。Direct VPC Egress + Cloud NAT + ファイアウォールルールによる外部APIアクセス構成を含みます。
