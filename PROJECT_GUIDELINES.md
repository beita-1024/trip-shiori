...
- **PR本文へ集約**: Issue化しない小さな残件はPR本文の専用セクションに要約（任意）
- **Issue化の基準**: 複数PR横断・期限/担当が必要・影響が広い場合のみIssue化

### PR本文（任意・追記用セクション）

必要に応じて、以下のセクションをPR本文に追記できます。
...
### 設定の要点
- 設定: `/.coderabbit.yaml`（日本語レビューを有効化）
- 自動レビュー: PR作成/Push時に自動、Draftは既定で無効

### レビュー時の使い方
- CodeRabbit のレビューコメントを確認し、必要に応じて対応
- 再レビュー依頼時は差分リンクを添付

### Merge時のラベル運用（残件の可視化）
- 目的: マージ後に残った事項の可視性を高める
- 付与タイミング: マージ直前 or マージ後すぐ（実装者またはレビュワー）
- ラベル定義:
  - `has-remaining-items`（残件あり）
- 使い方: `is:pr is:merged label:has-remaining-items` で抽出
- 備考: 後続Issueを起票した場合でも、PR側のラベルは情報ハブとして維持
