	...
	@cd terraform/environments/$(TF_ENV) && \
		...
		if gcloud compute networks describe "$${NETWORK_NAME}" --project="$${PROJECT_ID}" --quiet >/dev/null 2>&1; then \
			...
			if gcloud compute networks subnets describe "$${SUBNET_NAME}" --region="$${REGION}" --project="$${PROJECT_ID}" --quiet >/dev/null 2>&1; then \
				if ! terraform state show "module.network.google_compute_subnetwork.main" >/dev/null 2>&1; then \
					echo "📥 Subnetworkをインポート: $${SUBNET_NAME}"; \
					terraform import "module.network.google_compute_subnetwork.main" "projects/$${PROJECT_ID}/regions/$${REGION}/subnetworks/$${SUBNET_NAME}" || true; \
				fi; \
			fi; \
			\
			# GlobalAddress（Private Service Range）のインポート && \
			GLOBAL_ADDRESS_NAME="$${PROJECT_NAME}-psr"; \
			if gcloud compute addresses describe "$${GLOBAL_ADDRESS_NAME}" --global --project="$${PROJECT_ID}" --quiet >/dev/null 2>&1; then \
				if ! terraform state show "module.network.google_compute_global_address.private_service_range" >/dev/null 2>&1; then \
					echo "📥 GlobalAddressをインポート: $${GLOBAL_ADDRESS_NAME}"; \
					terraform import "module.network.google_compute_global_address.private_service_range" "projects/$${PROJECT_ID}/global/addresses/$${GLOBAL_ADDRESS_NAME}" || true; \
				fi; \
			fi; \
			\
			# Service Networking Connectionのインポート && \
			SERVICE_NETWORKING="servicenetworking.googleapis.com"; \
			CONNECTION_KEY="projects/$${PROJECT_ID}/global/networks/$${NETWORK_NAME}:$${SERVICE_NETWORKING}"; \
			if gcloud services vpc-peerings list --network="$${NETWORK_NAME}" --project="$${PROJECT_ID}" --quiet 2>/dev/null | grep -q "$${SERVICE_NETWORKING}" || true; then \
				if ! terraform state show "module.network.google_service_networking_connection.private_vpc_connection" >/dev/null 2>&1; then \
					echo "📥 Service Networking Connectionをインポート: $${CONNECTION_KEY}"; \
					terraform import "module.network.google_service_networking_connection.private_vpc_connection" "$${CONNECTION_KEY}" || true; \
				fi; \
			fi; \
		fi && \
		\
